        ...
        <div className="h-1/2 border-t-2 flex flex-col justify-around">
            <div className="flex 3xl:flex-row flex-col gap-1 justify-around columns-3">
            {
                itemsList.map((item,index)=>(
                    ...
                    <div key={index} className="flex items-center hover:border-b-2 hover:border-black transition-all duration-200 ease-in hover:pb-1 hover:bg-slate-300">
            ...
            <div className="flex justify-center items-end">
                <button onClick={()=>handleVerification("NameCurrency")} className='px-6 py-2 font-semibold text-white border-b-4 rounded shadow-lg bg-cyan-500 border-cyan-800 shadow-cyan-600/50 hover:border-cyan-600 md:text-base lg:text-lg xl:text-xl 2xl:text-2xl lg:px-12 lg:py-3'>check</button>
            </div>
        </div>      
    </>
  )
