    ...
    <>
        ...
        <div className="h-3/5 border-t-2 flex flex-col justify-around">
            <div className="gap-1 justify-around flex flex-col 3xl:flex-row lg:grid lg:grid-cols-2 landscape:flex-row">
                {
                    itemsList.map((item,index)=>(
                        <div key={index} className="flex items-center hover:bg-gray-400 transition-all duration-200 ease-in hover:pb-1 rounded-md px-2">
            ...
            <div className="flex justify-center items-end">
                <button onClick={()=>handleVerification("NameCurrency")} className='font-semibold text-white border-b-4 rounded shadow-lg bg-red-500 border-cyan-800 shadow-cyan-600/50 hover:border-cyan-600 px-16 py-2'>check</button>
            </div>
        </div>      
    </>
  )
