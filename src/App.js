...
// import {chooseRandomItems,chooRandomItem} from './utils/gameLogic.js'
import React, { useState, useEffect } from 'react';
import { db } from './utils/data/db.js';
import getData from "./utils/connectToAPI.js"
const { chooseRandomItems, chooRandomItem } = require('./utils/gameLogic.js');
...
function App() {
  ...
  useEffect(() => {
    ...
    const selectedItems = chooseRandomItems(optionsNumber,chooRandomItem,db)
    setItemsSelected(selectedItems)
    const randomForSolution = Math.floor(Math.random()*itemsSelected.length)
  ...
  return (
    <div className="App h-screen">
      <header className='bg-amber-400 w-full h-1/5 flex flex-col justify-center items-center'>
        <div className="h-1/2 flex items-center bg-black text-white w-full">ttt</div>
        <div className="h-1/2 flex items-center">Your Score: {Score}/{totalTries}</div>
      </header>
      <main className='w-full h-3/5 flex flex-col items-center justify-center'>
        <div className="border-8 rounded-2xl border-y-red-500  shadow-2xl w-1/2 h-3/4 px-4 py-2 border-x-amber-400 flex flex-col">
          <div className="h-1/2 flex justify-center my-5 ">
            <img className='w-32 border-2' src={itemsSelected[solution]?.flags.svg} alt="flag"></img>
          </div>
        ...
        <div className="mt-8 mx-6 my-2">
         ...
         {/* {JSON.stringify(itemsSelected[solution])} */}

      </main>
      <footer className='w-full bg-red-500 text-white h-1/5 flex justify-center items-center'>
        <div className=" ">
