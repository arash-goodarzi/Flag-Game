...
function App() {
    ...
    useEffect(() => {
        ...
        const loadingFromApi = async () => {
            const { selectedItems, randomForSolution } =
                await chooseRandomItems(optionsNumber)
    ...
    const handleVerification = (flag) => {
        ...
        if (flag === 'FlagName') {
            if (
                userSelected.name.common === itemsSelected[solution].name.common
            ) {
            ...
            if (
                itemsSelected[solution]['currencies'] === null ||
                itemsSelected[solution]['currencies'] === undefined ||
                Object.keys(itemsSelected[solution]['currencies']).length === 0
            ) {
                if (userSelected === 'None') setScore((prev) => prev + 1)
            } else {
                if (
                    itemsSelected[solution]['currencies'][
                        Object.keys(itemsSelected[solution]['currencies'])[0]
                    ]?.name
                ) {
                    if (
                        itemsSelected[solution]['currencies'][
